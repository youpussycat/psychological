...
const RouterPath = "/BackOrganization/AccountManagement/"

export const __AccountManagement = [];//用于暴露组件
for (const i of ComponentAttribute)//遍历上面的组件数组，用于生成暴露组件所需要的属性
    __AccountManagement.push({
        path: RouterPath + i,
        name: i,
        //import函数在webPack中无法只使用变量，只能字面量与常量结合使用或是只是用字面量,似乎插值语法也用不了.const是常变量
        component: () => import("../../components/BackOrganization/AccountManagement/"+i)
    });
